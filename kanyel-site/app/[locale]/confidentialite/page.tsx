...
export default async function PrivacyPolicyPage({
  ...
  const sections = [1, 2, 3, 4].map((i) => ({
    title: t(`privacy${i}Title` as never),
    body: t(`privacy${i}Body` as never),
  }));
      ...
      <main className="bg-bg py-20 sm:py-24">
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          <Reveal>
            ...
            <h1 className="mt-4 font-display text-3xl font-semibold text-navy sm:text-4xl">{t("privacyTitle")}</h1>
            <p className="mt-4 text-sm leading-relaxed text-ink-dim">{t("privacyIntro")}</p>
