    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
      <div className="lg:col-span-2">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl font-semibold text-admin-text">Demandes de devis</h2>
          {items.length > 0 && (
            <button
              onClick={exportCsv}
        ...
        <ul className="mt-4 space-y-2">
            ...
            <li key={item.id}>
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-admin-text-dim">{item.department_title || "Devis général"}</p>
                </div>
          ...
          {!loading && items.length === 0 && (
            <p className="text-sm text-admin-text-dim">Aucune demande de devis reçue pour le moment.</p>
          )}
        </ul>
      </div>
          ...
          <div className="rounded-3xl bg-admin-surface p-6 shadow-soft">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-admin-text-dim">{selected.department_title || "Devis général"}</p>
              </div>
              <button
            ...
            <dl className="mt-5 space-y-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-wide text-admin-text-dim">Budget</dt>
                  <dd className="text-admin-text-dim">{BUDGET_LABELS[selected.budget] || selected.budget}</dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold uppercase tracking-wide text-admin-text-dim">Délai souhaité</dt>
                  <dd className="text-admin-text-dim">{TIMELINE_LABELS[selected.timeline] || selected.timeline}</dd>
                </div>
              </div>
              <div>
                <dt className="text-xs font-semibold uppercase tracking-wide text-admin-text-dim">Description du projet</dt>
                <dd className="whitespace-pre-line text-admin-text-dim">{selected.description}</dd>
              </div>
              <div>
